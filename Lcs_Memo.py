...
# DSA --> LCS --> by Recursion (Memoization)
...
# Now we will solve the same problem with memoization

def LCS_by_Memoization(str1,str2):

    l_seq1, l_seq2 = len(str1), len(str2)

    matrix = [ [-1 for c in range(l_seq2 + 1) ] for r in range(l_seq1 + 1) ] 

    # 1st x-axis represents the seq2 and 1st y-axis represents the seq2 ( m = l_seq2 + 1 , n = l_seq1 + 1 )
    for r in matrix:
        print(r)
    ...
    #  l_1 and l_2 represents the length of corresponding
        ...
        if (seq1[l_1 - 1] == seq2[l_2 - 1]):
            ...
            matrix[l_1][l_2] =  1 + LCS_by_recursion(seq1,seq2,l_1-1,l_2-1)
            ...
            matrix[l_1][l_2] = max(first_choice,second_choice)

        return matrix[l_1][l_2]    
        

    LCS_by_recursion(str1,str2,l_seq1,l_seq2);

    return matrix[l_seq1][l_seq2]
...
if __name__ == '__main__':
    ...
    print(LCS_by_Memoization(str1,str2))
